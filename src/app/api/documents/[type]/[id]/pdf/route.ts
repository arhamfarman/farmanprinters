 ...
 * every "Download PDF" button in the dashboard links to directly (a plain
 * <a href>, not a fetch()), so the browser's native PDF viewer/download
 * handles it instead of the app buffering the whole file in JS.
 */
export async function GET(_request: NextRequest, { params }: { params: { type: string; id: string } }) {
  ...
  const buffer = await renderDocumentPdf(params.type as PdfDocumentType, params.id, session.pressId);
